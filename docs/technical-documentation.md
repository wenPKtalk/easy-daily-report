...
> **版本:** 1.1 (MVP1 - Multiple Agent + 策略模式)  
> **日期:** 2026-04-30  
> **作者:** Pengkunwen  
...
│  │  report generate / report generate-today                │ │
│  └──────────────────────┬──────────────────────────────────┘ │
├─────────────────────────┼────────────────────────────────────┤
│                Application Layer (策略模式)                   │
│                                                              │
│  ┌──── GenerateAgent 接口（策略）────────────────────────┐  │
│  │                                                       │  │
│  │  ┌─────────────────────┐  ┌────────────────────────┐ │  │
│  │  │ GenerateReportUseCase│  │ MultiAgentOrchestrator │ │  │
│  │  │  (单 Agent 策略)     │  │  (多 Agent 并行策略)    │ │  │
│  │  └──────────┬──────────┘  └──────────┬─────────────┘ │  │
│  └─────────────┼──────────────────────── ┼───────────────┘  │
│                │                         │                   │
│  ┌─────────────▼─────────────────────────▼───────────────┐  │
│  │  AgentRouter (策略选择器，待完善)                       │  │
│  └───────────────────────────────────────────────────────┘  │
├─────────────────────────┬────────────────────────────────────┤
│     Domain Layer        │                                    │
│  ┌──────────────────────┴──────────────────────────────────┐ │
│  │  Port: ReportGenerator / ReportStore / GitPort / JiraPort│ │
│  │  Model: DailyReport / CodeChange / JiraIssueInfo         │ │
│  │  Model: ReportRequest                                    │ │
│  └──────────────────────┬──────────────────────────────────┘ │
├─────────────────────────┼────────────────────────────────────┤
...
└────────┬────────┘
         ▼
┌──────────────────────┐
│ GenerateReportUseCase│──── 调用 ReportGenerator + ReportStore
└────────┬─────────────┘
         ▼
┌─────────────────┐
│ ReAct Agent     │◄─── System Prompt + RAG 历史日报
│ (LangChain4j)   │
│                 │──► Action: GitTool.getCommitDiff()
│                 │◄── Observation: diff 内容
│                 │──► Action: JiraTool.getJiraIssue()
...
└────────────┘  └─────────────┘
    ...
    └────────┬────────┘
             ▼
    ┌─────────────────┐
    │ ReportGenerator │──── 整合 JSON 结果 → Markdown 日报
    │ Agent           │
    └────────┬────────┘
             ▼
...
│
├── agent/                                    # Agent 层 ★ MVP1 新增
│   └── subagents/                            # LangChain4j AiService 接口
│       ├── GitDiffAnalyzerAgent.java         #   代码变更分析 Sub-Agent
│       ├── JiraAnalyzerAgent.java            #   Jira 业务分析 Sub-Agent
...
├── application/                              # 应用层
│   └── usecase/
│       ├── GenerateAgent.java                #   ★ 策略接口（MVP1 新增）
│       ├── AgentLevel.java                   #   Agent 级别枚举（SINGLE/SAMPLE_MULTIPLE/...）
│       ├── AgentRouter.java                  #   策略路由器（待完善）
│       ├── GenerateReportUseCase.java        #   单 Agent 策略实现
│       └── MultiAgentOrchestrator.java       #   ★ 多 Agent 并行策略实现（MVP1 新增）
│
├── infrastructure/                           # 基础设施层
│   ├── ai/                                   # AI Agent (单 Agent 路径)
│   │   ├── DailyReportAgent.java             #   AiServices 接口（ReAct 模式）
...
│
└── shell/                                    # 接口层
    └── DailyReportCommands.java              #   CLI 命令定义（含 generate-today）
```
...
项目支持两种 Agent 策略，均通过 `GenerateAgent` 接口统一抽象：
...
### 7.1 策略一：单 Agent (ReAct 模式)

**入口：** `GenerateReportUseCase` → `AgentReportGenerator` → `DailyReportAgent`
...
**工作原理：** LangChain4j 的 `AiServices` 在运行时为此接口生成动态代理，自动处理 Tool 调用路由、ChatMemory 管理、RAG ContentRetriever 注入和 ReAct 推理循环。

**组装（LangChain4jConfig）：**
...
### 7.2 策略二：Multi-Agent 并行模式 (MVP1)

**入口：** `MultiAgentOrchestrator`

三个专职 Sub-Agent 接口，均通过 `MultiAgentConfig` 的 `AiServices.builder()` 装配：

| Sub-Agent | 接口 | 职责 | 输出格式 |
|-----------|------|------|---------|
| `GitDiffAnalyzerAgent` | `analyze(diff)` | 分析代码变更，识别关键点和风险 | JSON |
| `JiraAnalyzerAgent` | `analyze(issueContent)` | 分析业务背景、需求和价值 | JSON |
| `ReportGeneratorAgent` | `generate(prompt, gitJson, jiraJson, date)` | 整合结果生成 Markdown 日报 | Markdown |

**并行编排（MultiAgentOrchestrator）：**

```java
// 步骤1: 并行执行 Git 和 Jira 分析
CompletableFuture<String> gitFuture =
    CompletableFuture.supplyAsync(() -> analyzeGitChanges(commitHash));
CompletableFuture<String> jiraFuture =
    CompletableFuture.supplyAsync(() -> analyzeJiraIssue(jiraKey));

// 步骤2: 等待所有分析完成
CompletableFuture.allOf(gitFuture, jiraFuture).join();

// 步骤3: 生成最终日报
return reportGeneratorAgent.generate(prompt,
    gitFuture.join(), jiraFuture.join(), todayDate);
```

**GitTool 与 JiraTool** 在 Multi-Agent 模式中作为普通 Spring Bean 注入，由 `MultiAgentOrchestrator` 直接调用（而非通过 LLM Tool Calling），分别向各 Sub-Agent 提供原始数据。
...
| 方法 | 描述 |
|------|------|
| `getCommitDiff(commitHash)` | 获取 commit 详情 + diff |
| `getRecentCommits(count)` | 列出最近 N 条 commit |

#### JiraTool

| 方法 | 描述 |
|------|------|
| `getJiraIssue(issueKey)` | 获取 Issue 完整信息 |

### 7.4 AgentLevel 与 AgentRouter

```java
public enum AgentLevel {
    SINGLE,            // 单 Agent ReAct 模式
    SAMPLE_MULTIPLE,   // 并行 Multi-Agent 模式
    COORDINATOR_AGENT  // 预留：Master-Sub 协调模式
}
```

`AgentRouter` 是策略选择器，负责根据 `AgentLevel` 返回对应的 `GenerateAgent` 实现。当前为待完善状态，默认使用 `GenerateReportUseCase`（单 Agent 策略）。

### 7.5 ChatMemory 配置

- **类型:** `MessageWindowChatMemory`
- **单 Agent 窗口:** 20 条消息
- **Sub-Agent 窗口:** 10 条消息（各 Agent 独立内存）
...
```
report generate-today [OPTIONS]  自动生成今日日报（推荐）
report generate [OPTIONS]        基于指定 commit 生成日报
report help                      显示帮助信息
```
...
| 参数 | 短标志 | 长标志 | 说明 | 示例 |
|------|--------|--------|------|------|
| Jira Issue | `-j` | `--jira` | Jira Issue Key（可选） | `PROJ-123` |
| Repo Path | `-p` | `--repo` | Git 仓库路径（可选） | `/path/to/repo` |
...
**`report generate-today` 实现原理：** 调用 `GitPort.getTodayCommits()` 获取当天所有提交，自动构建 commit 范围字符串（最早..最新），再走标准的 `GenerateReportUseCase` 流程。
...
`GenerateAgent` 接口是 MVP1 的核心设计，使日报生成策略可以独立演进：
...
│                       │            │                          │
│  ReportGenerator ─► DailyReportAgent │  GitDiffAnalyzerAgent  │
│  (ReAct + RAG)        │            │  JiraAnalyzerAgent       │
...
```
         ...
         Facade ────│ DailyReportCmds │
                    └────────┬────────┘
                             │
                    ┌────────▼────────┐
                    │  GenerateAgent  │ ← Strategy 接口
                    └──┬──────────┬──┘
           Strategy    │          │    Strategy
               ┌───────▼──┐  ┌───▼─────────────┐
               │UseCase   │  │MultiAgentOrchest │
               │(单Agent) │  │(并行多Agent)     │
               └───┬──────┘  └───┬─────────────┘
         Adapter   │    Orchestrator│
           ┌───────▼──┐    ┌──────▼──────────┐
           │AgentReport│   │GitDiffAnalyzer  │
           │Generator  │   │JiraAnalyzer     │
           │ Proxy ──► │   │ReportGenerator  │ ← Proxy (AiServices)
           │DailyReport│   └─────────────────┘
           │Agent      │
           └─────┬─────┘
     Repository  │
...
| 类 | 单一职责 |
|-----|---------|
| `GenerateReportUseCase` | 只编排单 Agent 生成流程 |
| `MultiAgentOrchestrator` | 只编排多 Agent 并行流程 |
| `AgentRouter` | 只负责策略选择 |
...
- 新增 Agent 策略 → 实现 `GenerateAgent` 接口，无需修改现有代码
...
### MVP1 — Multiple Agent + 策略模式

**核心变更：**

| 新增内容 | 说明 |
|---------|------|
| `GenerateAgent` 接口 | 策略模式抽象，统一两种生成路径 |
| `MultiAgentOrchestrator` | 并行 Multi-Agent 编排器，`CompletableFuture` 并行执行 |
| `agent/subagents/` 包 | 三个专职 Sub-Agent 接口（Git/Jira/Report） |
| `MultiAgentConfig` | Sub-Agent Bean 装配配置 |
| `AgentLevel` 枚举 | `SINGLE` / `SAMPLE_MULTIPLE` / `COORDINATOR_AGENT` |
| `AgentRouter` | 策略路由器（框架已就绪，路由逻辑待完善） |
